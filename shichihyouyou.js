...
(function (window, document) {
  ...
  var months = ['January', 'February', 'March', 'April', 'May', 'June', 'July', '８月', 'September', 'October', 'November', 'December'];
...
function onCalFrameClick(e) {
    ...
    this.setViewDate(this.getCurrentDate());
	  ...
	  //
	  shichiyouhyou.isLeapYear = function (year) {
	    if (year === undefined) throw new ReferenceError('Year not defined.');
	    return ((year%4==0) && (year%100!=0)) || (year%400==0);
	  };

	  shichiyouhyou.getDaysInMonth = function(month, year) {
	    return ((month !== 1) ? (daysInMonth[month]) : (shichiyouhyou.isLeapYear(year) ? 29:28));
	  };

	  shichiyouhyou.isEqualDate = function (date1, date2) {
	  ...
	  shichiyouhyou.prototype.getCurrentDate = function() {
	    return this.current;
	  };

	  shichiyouhyou.prototype.setCurrentDate = function(date) {
	    this.current = date;
	  };

	  shichiyouhyou.prototype.setViewDate = function (date) {
	  ...
	  shichiyouhyou.prototype.add = function (type, time) {
	  ...
	  shichiyouhyou.prototype.subtract = function (type, time) {
	    this.add.call(this, type, -time);
	  };

	  shichiyouhyou.prototype.create = function () {
	    var cache = shichiyouhyou.cache;
	    var curr = this.getCurrentDate();
	    ...
	    var days  = shichiyouhyou.getDaysInMonth(+month, +year);
	  ...
	  shichiyouhyou.prototype.render = function () {
	    var cal = shichiyouhyou.cache;
	    var curr = this.getCurrentDate();
	    ...
	    for (var i = 0, monthLen = weeks.length; i < monthLen; i++) {
	      ...
	      for (var j = 0, weekLen = week.length; j < weekLen; j++) {
	        ...
	        if (day) {
	          ...
	          if (shichiyouhyou.isEqualDate(this.today, new Date(year,month,day))) {
	  ...
	  window.onload = function () {
	    new shichiyouhyou(document.getElementById('cal'));
	  }
	})(window, document);
